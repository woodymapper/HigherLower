...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        while (p)
        {
            ...
            int Card2 = r.Next(1, 13);
            while (Card1 == Card2) Card2 = r.Next(1, 13);
            CardType(Card1, 1);
            Console.WriteLine("Czy Karta 1 jest (W)iększa czy (M)niejsza w porównaniu do karty 2?(domyślnie jest higher):");
            bool which = true;
            if (Console.ReadLine() == "M") which = false;
            CardType(Card2, 2);
            if (!which && Card2 > Card1 || which && Card1 > Card2) Console.WriteLine("Wygrałeś wpisz end aby zakończyć lub kontynuj.");
